...
namespace indigo {
    ...
    template <typename T> Vec<T>::Vec(size_t len) {
    ...
    template <typename T> Vec<T>::~Vec() {
        ...
        if (!takeMyData) free(data);
    }
    template <typename T> T& Vec<T>::operator[](size_t index) {
        return data[index];
    }
    template <typename T> void Vec<T>::Append(T value) {
        data[len] = value;
        len++;
        if (len >= allocated) {
            allocated += VEC_ALLOC_BLOCK_SIZE;
