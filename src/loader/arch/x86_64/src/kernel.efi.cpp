...
Kernel::Kernel(
    ...
    for (int i = 0; i < header->progHeaderTableLength; i++) {
        elf::ProgHeader* prog_header =
            (elf::ProgHeader*)((uint8_t*)file.data + header->progHeaderPos +
                               i * header->progEntrySize);
        if (prog_header->type == PH_LOAD) {
            printf("current pA is 0x%x\n", prog_header->physAddr);
            memcpy(
                (void*)prog_header->physAddr,
                (uint8_t*)file.data + prog_header->fileOffset,
                prog_header->fileSize
            );
            memset(
                ((uint8_t*)prog_header->physAddr + prog_header->fileSize), 0,
                prog_header->memSize - prog_header->fileSize
            );
            ...
            kSegs.Append(
                KernelSegmentLoc{
                    .physLoc = prog_header->physAddr,
                    .len     = prog_header->memSize
                }
            );
        }
    }
    ...
    // we really should page-align it so we can mark not-exe in the future but
    // that's future me problem
    // TODO: see above
    // pad it out a little since kernel __attribute__ ((align)) is a bit dodgy
    // and likes to trample me give it a KiB in future, if something pagefaults
    // randomly it's probably kargs->framebuffer being trampled, so increase
    // this value
    ...
    for (int i = 0; i < tHeader->progHeaderTableLength; i++) {
        elf::ProgHeader* prog_header =
            (elf::ProgHeader*)((uint8_t*)tFile.data + tHeader->progHeaderPos +
                               i * tHeader->progEntrySize);
        if (prog_header->type == PH_LOAD) {
            memcpy(
                (void*)prog_header->physAddr,
                (uint8_t*)tFile.data + prog_header->fileOffset,
                prog_header->fileSize
            );
            memset(
                ((uint8_t*)prog_header->physAddr + prog_header->fileSize), 0,
...
void Kernel::Run(size_t argc, char** argv) {
    ...
    // TODO: acc figure this line out. since it's not 8, and one memory alloc
    // isn't the size of efi_memory_descriptor_t
    uint8_t* mmapOrig =
        (uint8_t*)malloc(mmapOrigSize + sizeof(efi_memory_descriptor_t) * 8);
    ...
    MemoryDescriptor* mmapStart =
        (MemoryDescriptor*)((size_t)args + args->totalSize - mmapSize -
                            kSegSize);
    int acc_len = 0;
    ...
    for (int i = 0; i < origInfo.fullSize; i += origInfo.descriptorSize) {
        efi_memory_descriptor_t* descriptor =
            (efi_memory_descriptor_t*)(mmapOrig + i);
