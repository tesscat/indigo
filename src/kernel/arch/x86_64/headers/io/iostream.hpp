...
namespace io {
    ...
    inline sstream& operator<<(sstream& stream, const char* text) {
        stream.write(text);
        return stream;
    }
    inline sstream& operator<<(sstream& stream, String& text) {
        stream.write(text);
        return stream;
    }
    inline sstream& operator<<(sstream& stream, const char c) {
        stream.write(c);
        return stream;
    }
    sstream& operator<<(sstream& stream, int64_t v);
    sstream& operator<<(sstream& stream, uint64_t v);
    inline sstream& operator<<(sstream& stream, int v) {
        return stream << (int64_t)v;
    }
