...
#define panic(msg)                                                             \
    util::_panic(                                                              \
        msg, __FILE__, __COMPTIME_STRINGIFY(__LINE__), __PRETTY_FUNCTION__     \
    );

#define unimplemented() panic("UNIMPLEMENTED")

    // [[noreturn]] void _panic(const char* file, const char* line, const char*
    // func) noexcept;
    ...
    /**
     * @brief Rounds away-from-zero like `roundUp`, but is faster when
     * multipleToRoundTo is a power of 2
     *
     * @param numToRound The number to round
     * @param multipleToRoundTo The multiple to round to
     * @return The rounded number
     */
    inline uint64_t
    roundUpToPowerOfTwo(uint64_t numToRound, uint64_t multipleToRoundTo) {
        // check we have not been lied to
        if (!(multipleToRoundTo &&
              ((multipleToRoundTo & (multipleToRoundTo - 1)) == 0)))
