...
namespace util {
    ...
    template <typename T> bool Vec<T>::Contains(T& value) {
        for (uint64_t i = 0; i < len; i++) {
    ...
    template <typename T> Vec<T>::Vec(uint64_t len_) : len{len_} {
        if (len != 0) {
            allocated =
                (len - (len % VEC_ALLOC_BLOCK_SIZE)) + VEC_ALLOC_BLOCK_SIZE;
            data = (T*)kmalloc(allocated * sizeof(T));
        } else {
            allocated = 0;
            data      = nullptr;
    ...
    template <typename T> void Vec<T>::Trim() {
        allocated = len;
        if (allocated == 0) {
            if (data) {
                kfree(data);
    ...
    template <typename T> void Vec<T>::ApproxTrim() {
    ...
    template <typename T> Vec<T>::~Vec() {
        ...
        if (!takeMyData && data) kfree(data);
        data = nullptr;
        len  = 0;
    }
    template <typename T> T& Vec<T>::operator[](uint64_t index) {
        return data[index];
    }
    template <typename T> void Vec<T>::Append(const T& value) {
        if (len >= allocated) {
