...
// 10 is undefined
//
// This means that _1 <=> not worth searching for the current size and vice
// versa, and 0_ <=> worth searching for smaller sizes and vice versa.
//
// There can be desync!! We can mark a higher level page as Completely Taken and
// all the lower pages don't need updating. The highest level must always hold
// true. We use 4KiB, 128KiB, 2M, 64M, 128M sections
...
const uint64_t sectSizes[] =
    {4 * KiB, 128 * KiB, 2 * MiB, 64 * MiB /* , 128*MiB */};
...
namespace memory {
    ...
    void mark2mAsUsed(uint64_t addr) {
        ...
        // check again
        // no mask this time
        if (!((maps[2][idx_2m(addr)] & fully_used_64bits) ==
              fully_used_64bits)) {
            or64mAddr(addr, 0b01);
            return;
        }
    ...
    void mark128kAsUsed(uint64_t addr) {
        ...
        if (!((maps[1][idx_128k(addr)] & fully_used_64bits & mask) ==
              (fully_used_64bits & mask))) {
    ...
    void free2m(uint64_t addr) {
        ...
        if (!((maps[2][idx_2m(addr)]) == 0)) {
            ...
            // TODO: make specific unmarkFullUse so we don't need to do two BM
            // operations
            clear64mAddr(addr);
            or64mAddr(addr, 0b01);
            return;
    ...
    void markBlockAsUsedAt128k(uint64_t start, uint64_t size) {
        ...
        if (size < 128 * KiB) return markBlockAsUsedAt4k(start, size);
        // get the low-slice
        uint64_t low_128k_addr = util::roundUpToPowerOfTwo(start, 128 * KiB);
        uint64_t low_size      = low_128k_addr - start;
        if (low_size != 0) markBlockAsUsedAt4k(start, low_size);
        // mark out any blocks of our size
        uint64_t curr_size = 0;
        for (; curr_size + 128 * KiB <= (size - low_size);
             curr_size += 128 * KiB) {
    ...
    void markBlockAsUsedAt2m(uint64_t start, uint64_t size) {
        // if it has too small a size, immediately delegate
        if (size < 2 * MiB) return markBlockAsUsedAt128k(start, size);
        // get the low-slice
        uint64_t low_2m_addr = util::roundUpToPowerOfTwo(start, 2 * MiB);
        uint64_t low_size    = low_2m_addr - start;
        if (low_size != 0) markBlockAsUsedAt128k(start, low_size);
        // mark out any blocks of our size
        uint64_t curr_size = 0;
    ...
    void markBlockAsUsedAt64m(uint64_t start, uint64_t size) {
        // if it has too small a size, immediately delegate
        if (size < 64 * MiB) return markBlockAsUsedAt2m(start, size);
        // get the low-slice
        uint64_t low_64m_addr = util::roundUpToPowerOfTwo(start, 64 * MiB);
        uint64_t low_size     = low_64m_addr - start;
        if (low_size != 0) markBlockAsUsedAt2m(start, low_size);
        // mark out any blocks of our size
        uint64_t curr_size = 0;
        for (; curr_size + 64 * MiB <= (size - low_size);
             curr_size += 64 * MiB) {
    ...
    void freeBlockAt128k(uint64_t start, uint64_t size) {
        // if it has too small a size, immediately delegate
        if (size < 128 * KiB) return freeBlockAt4k(start, size);
        // get the low-slice
        uint64_t low_128k_addr = util::roundUpToPowerOfTwo(start, 128 * KiB);
        uint64_t low_size      = low_128k_addr - start;
        if (low_size != 0) freeBlockAt4k(start, low_size);
        // mark out any blocks of our size
        uint64_t curr_size = 0;
        for (; curr_size + 128 * KiB <= (size - low_size);
             curr_size += 128 * KiB) {
    ...
    void freeBlockAt2m(uint64_t start, uint64_t size) {
        // if it has too small a size, immediately delegate
        if (size < 2 * MiB) return freeBlockAt128k(start, size);
        // get the low-slice
        uint64_t low_2m_addr = util::roundUpToPowerOfTwo(start, 2 * MiB);
        uint64_t low_size    = low_2m_addr - start;
        if (low_size != 0) freeBlockAt128k(start, low_size);
        // mark out any blocks of our size
        uint64_t curr_size = 0;
        for (; curr_size + 2 * MiB <= (size - low_size); curr_size += 2 * MiB) {
    ...
    void freeBlockAt64m(uint64_t start, uint64_t size) {
        // if it has too small a size, immediately delegate
        if (size < 64 * MiB) return freeBlockAt2m(start, size);
        // get the low-slice
        uint64_t low_64m_addr = util::roundUpToPowerOfTwo(start, 64 * MiB);
        uint64_t low_size     = low_64m_addr - start;
        if (low_size != 0) freeBlockAt2m(start, low_size);
        // mark out any blocks of our size
        uint64_t curr_size = 0;
        for (; curr_size + 64 * MiB <= (size - low_size);
             curr_size += 64 * MiB) {
    ...
    uint64_t findFirstPartial2mIdxWithin64m(uint64_t idx_64m) {
        ...
        if ((currBatch & fully_used_64bits) != 0) {
            ...
            while ((0b10 & currBatch) == 0) {
                ...
                currBatch = currBatch >> 2;
                if (shiftIdx > 32) {
                    return findFirstPartial2mIdxWithin64m(
                        findFirstPartial64mIdx(idx_64m + 1)
                    );
                }
            }
    ...
    uint64_t findFirstFullyFree2mIdxWithin64m(uint64_t idx_64m) {
        ...
        if ((currBatch & fully_used_64bits) != 0) {
            ...
            while ((0b01 & currBatch) == 0) {
                ...
                currBatch = currBatch >> 2;
                if (shiftIdx > 32) {
                    return findFirstFullyFree2mIdxWithin64m(
                        findFirstPartial64mIdx(idx_64m + 1)
                    );
                }
            }
    ...
    uint64_t findFirstPartial128kIdxWithin2m(uint64_t idx_2m) {
        // there are 16 128ks within one 2m, but each 16 takes one half of a
        // uint64_t
    ...
    void initPhysAllocator() {
        uint64_t kernel_phys_start =
            (uint64_t)&_kernel_virt_start - KERNEL_OFFSET;
        uint64_t kernel_phys_end = (uint64_t)&_kernel_virt_end - KERNEL_OFFSET;
        // how big will the bitmaps need to be?
        uint64_t memSize = memory::memMap[memory::memMapLen - 1].start +
                           memory::memMap[memory::memMapLen - 1].len;
        uint64_t mapSize = 0;
        ...
        // the area immediately above the kernel in physical memory is mapped,
        // so we can use it as a kind-of scratch buffer it's very useful, for
        // instance, now page align it too since why not
        uint64_t* map_4k =
            (uint64_t*)(graphics::screen::backBufferEnd +
                        (4096 - (graphics::screen::backBufferEnd % 4096)));
        ...
        // next up is to get variables for their length, zero them, then mark
        // what's already in use (kernel + this map itself + page tables) zero
        // them
        ...
        // - low memory is dodgy, QEMU doesn't like it either so we prevent us
        // using it
        // - in case we need anything in the future that is Absolutely
        ...
        markBlockAsUsedAt4k(
            kernel_phys_start, kernel_phys_end - kernel_phys_start
        );
