...
namespace memory {
    ...
    // just swap allocate4kPage() in the below function for something that
    // allocates + maps it which would be a getOrCreateSubdirInKernelSpace which
    // uses the slab alloc instead of allocate4kPage and then very pointedly
    // un-allocates them later (important!!)
    ...
    PageEntryBase* getOrCreateSubdir(PageEntryBase* entry, uint64_t flags) {
        ...
        // since .clear() zeroes it, we assume if addr = 0 then the entry
    ...
    template <uint64_t (*allocator)()>
    PageDirEntry*
    locateOrAllocatePDE(PageMap map, uint64_t virtAddr, uint64_t createFlags) {
    ...
    // the memory allocated for the map is not mapped itself, we just hope since
    // the kernel is id-mapped that it works fine
    template <uint64_t (*allocator)()>
    PageTableEntry*
    locateOrAllocatePTE(PageMap map, uint64_t virtAddr, uint64_t createFlags) {

        PageDirEntry* l2 =
            locateOrAllocatePDE<allocator>(map, virtAddr, createFlags);

        PageTableEntry* l1_dir =
            (PageTableEntry*)getOrCreateSubdir<allocator>(l2, createFlags);
        PageTableEntry* l1 = &l1_dir[PTE_IDXOF(virtAddr)];
        return l1;
    }

    template <uint64_t (*allocator)()>
    PageDirEntry* map2MiB(
        PageMap map,
        uint64_t virtAddr,
        uint64_t physAddr,
        ...
        // get the PDE
        PageDirEntry* pde = locateOrAllocatePDE<allocator>(
            map, virtAddr, flags & (~PAGE_BIG_PAGE)
        );
        pde->clear();
    ...
    PageTableEntry* map4KiB(
        ...
        // get the PTE
        PageTableEntry* pte =
            locateOrAllocatePTE<allocator>(map, virtAddr, flags);
        pte->clear();
    ...
    uint64_t slab_allocator() {
        ...
        if (addr == 0) panic("Kernel paging slab out of memory");
        return addr;
    }
    ...
    void initPageAllocator() {
        ...
        // 511 is a bit of a guess, ideally we'd have the slab alloc solve for
        // the maximum it can fit
        // TODO: have slab solve for how much to allocate for the bitmap
        slab = libmem::SlabAllocator<PageTable>(
            (PageTable*)initial_paging_scratch, 511
        );
        // Okay turns out we need to make a kpgtable now, since the trampoline
        // one will probably be overwritten, since we don't block it out in the
        // phys_alloc find a 4kb aligned addr in initial_paging_scratch
        ...
        // since the backbuffer needs to be mapped we have to cope with that one
        // too
        ...
        while (curr_start_addr + 2 * MiB < curr_end_addr) {
            map2MiB<slab_allocator>(
                kpgtable, curr_start_addr, curr_start_addr - KERNEL_OFFSET
            );
        ...
        curr_end_addr   = util::roundUpToPowerOfTwo(
            ((uint64_t)kargs->framebuffer +
             (kargs->fbPitch * kargs->fbHeight) * sizeof(uint32_t)),
            4 * KiB
        );
        ...
        map2MiB<slab_allocator>(
            kpgtable, (uint64_t)initial_paging_scratch,
            (uint64_t)initial_paging_scratch
        );
    ...
    void
    kernelMap4KiBBlock(uint64_t virtAddr, uint64_t physAddr, uint64_t flags) {
        __asm__ volatile("invlpg %0" : : "rm"(virtAddr));
        // just map it
        map4KiB<slab_allocator>(
            kpgtable, (uint64_t)virtAddr, (uint64_t)physAddr, flags
        );
        lcr3((uint64_t)kpgtable);
    }
    ...
    // this is a bit more complex, since we need to track the tables all the way
    // down + check if they empty
    ...
    void
    kernelMap2MiBBlock(uint64_t virtAddr, uint64_t physAddr, uint64_t flags) {
        // TODO: TLB shootdown??
