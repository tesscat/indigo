...
namespace memory {
    ...
    void initSystemMap(MemoryDescriptor* mDescs, size_t mDescsCount) {
        ...
        while (i < mDescsCount) {
            ...
            size_t j = i;
            while (j + 1 < mDescsCount && isUsableMemory(mDescs[j + 1].type) &&
                   mDescs[j].physStart + (4096 * mDescs[j].nrPages) ==
                       mDescs[j + 1].physStart) {
                j++;
            }
            ...
            if (i == j) {
                ...
                mmaps[mmaps_idx].type  = Free;
                mmaps[mmaps_idx].start = mDescs[i].physStart;
                mmaps[mmaps_idx].len   = mDescs[j].physStart -
                                       mDescs[i].physStart +
                                       4096 * mDescs[j].nrPages;
            }
            mmaps_idx += 1;
        ...
        while (j < mmaps_idx) {
            ...
            if ((curr_start <= kStart && curr_end >= kStart) ||
                ...
                curr_idx++;
                // in case kernel spills out of one segment (it shouldn't
                // really)
                while (kEnd > (mmaps[j].start + mmaps[j].len) &&
                       j < mmaps_idx) {
                    j++;
                }
