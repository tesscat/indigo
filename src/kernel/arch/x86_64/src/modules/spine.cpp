...
namespace modules {
    ...
    void initSpine() {
        ...
        elf::SectHeader* sectTable =
            (elf::SectHeader*)(((uint64_t)kHeader) + kHeader->sectHeaderPos);
        elf::SectHeader* strTabHeader =
            (elf::SectHeader*)((uint64_t(sectTable)) +
                               kHeader->sectHeaderNamesIndex *
                                   kHeader->sectEntrySize);
        char* shStrTab =
            (char*)(((uint64_t)kHeader) + strTabHeader->fileOffset);
        char* strTab = nullptr;
        // First, find the Actual strtab
        elf::SectHeader* curr = sectTable;
        for (int i = 0; i < kHeader->sectHeaderTableLength;
             i++, *((uint64_t*)&curr) += kHeader->sectEntrySize) {
        ...
        for (int i = 0; i < kHeader->sectHeaderTableLength;
            ...
            if (curr->type == elf::SectHeaderType::SHT_SYMTAB) {
                ...
                elf::SymbolTableEntry* ste =
                    (elf::SymbolTableEntry*)(((uint64_t)kHeader) +
                                             curr->fileOffset);
                for (; (uint64_t)ste <
                       curr->fileSize + curr->fileOffset + (uint64_t)kHeader;
                     *((uint64_t*)&ste) += curr->entrySize) {
                    // if the value isn't zero, it's visible and globally
                    // linked;
                    if (ste->value != 0 &&
                        ste->visibility == elf::SymbolVisibility::STV_DEFAULT) {
                        // Find the name
