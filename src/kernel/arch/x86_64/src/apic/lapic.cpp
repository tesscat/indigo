...
namespace apic {
    ...
    namespace lapic {
#define helper_uint32(name, offs)                                              \
    uint32_t volatile* name = reinterpret_cast<uint32_t*>(LAPIC_BASE + offs);
#define helper_uint32_const(name, offs)                                        \
    uint32_t const volatile* name =                                            \
        reinterpret_cast<uint32_t*>(LAPIC_BASE + offs);
#define helper_lvt(name, offs)                                                 \
    LvtRegister volatile* name =                                               \
        reinterpret_cast<LvtRegister*>(LAPIC_BASE + offs);
        helper_uint32(apicId, 0x20) helper_uint32_const(apicVersion, 0x30) helper_uint32(taskPriority, 0x80) helper_uint32_const(arbitrationPriority, 0x90) helper_uint32_const(
            processorPriority,
            0xA0
        ) helper_uint32(eoi, 0xB0) helper_uint32_const(remoteRead, 0xC0) helper_uint32(logicalDestination, 0xD0) helper_uint32(destinationFormat, 0xE0) helper_uint32(spuriousInterruptVector, 0xF0)
            // TODO: the In-Service register and Trigger Mode Register and
            // Interrupt Request Register
            helper_uint32(errorStatus, 0x280) helper_uint32(
                icr0,
                0x300
            ) helper_uint32(icr1, 0x310) namespace lvt{
                helper_lvt(correctedMachineCheck, 0x2F0)
                    helper_lvt(timer, 0x320) helper_lvt(thermalSensor, 0x330)
                        helper_lvt(performanceMonitor, 0x340)
                            helper_lvt(lint0, 0x350) helper_lvt(lint1, 0x360)
                                helper_lvt(error, 0x370)
            } helper_uint32(timerInitialCount, 0x380) helper_uint32_const(timerCurrentCount, 0x390) helper_uint32(timerDivideConfiguration, 0x3E0)
    }
    ...
    void initLapic() {
        ...
        while ((uint64_t)curr < ((uint64_t)madt) + madt->header.Length) {
            switch (curr->type) {
                case acpi::ProcessorLocalApic: {
                    // check validity
                    tables::ProcessorLocalApic* pla =
                        (tables::ProcessorLocalApic*)curr;
                    if (pla->flags & 0b11) multi::nCpus += 1;
                    break;
                }
                ...
                case acpi::ProcessorLocalx2Apic: {
                    // check validity
                    tables::ProcessorLocalx2Apic* pla =
                        (tables::ProcessorLocalx2Apic*)curr;
                    if (pla->flags & 0b11) multi::nCpus += 1;
                    break;
                }
                default: {
        ...
        memory::kernelIdentityMap4KiBBlock(
            LAPIC_BASE, PAGE_PRESENT | PAGE_READ_WRITE | PAGE_WRITE_THROUGH
        );
        ...
        *lapic::timerDivideConfiguration = 0x3;
        ...
        // im not sure why we need to set bit 1 low, i think it disables the
        // speaker
    ...
    void apicSleep(uint64_t ms) {
        uint64_t otherMath =
            (*lapic::timerInitialCount - ((ticksPerSecond * ms) / 1000));
        uint64_t target =
            (*lapic::timerCurrentCount + otherMath) % *lapic::timerInitialCount;
        bool sign = target > *lapic::timerCurrentCount;
        while (*lapic::timerInitialCount < target || sign) {
            if (sign) sign = target > *lapic::timerCurrentCount;
            __asm__ volatile("pause" : : : "memory");
        }
    }
