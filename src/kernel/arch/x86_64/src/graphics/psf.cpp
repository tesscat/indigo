...
namespace graphics::psf {
    ...
    void
    putchar(uint8_t c, uint64_t cx_, uint64_t cy_, uint32_t fg, uint32_t bg) {
    ...
    void putcharExactly(
        ...
        uint8_t* glyph = (uint8_t*)&fontStart
            [(c > font->glyphCount ? 0 : c) * font->bytesPerGlyph];
