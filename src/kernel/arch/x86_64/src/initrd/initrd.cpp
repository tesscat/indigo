...
namespace initrd {
    int initrd() {
        ...
        // - unmount it (maybe?)
        //   we _should_ unmount it since it's taking up space we don't need it
        //   to
        memory::MemoryFile* initrdFile = new memory::MemoryFile(
            kargs->initrd, kargs->initrdLen, true, false
        );
