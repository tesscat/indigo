...
bool isUsedByCode(uint64_t addr) {
    ...
    for (int i = 0; i < kargs->kernelSegmentsCount; i++) {
        ...
        if (doesEncompass(
                kargs->kernelSegments[i].physLoc, kargs->kernelSegments[i].len,
                addr
            ) ||
            doesEncompass(
                kargs->kernelSegments[i].physLoc, kargs->kernelSegments[i].len,
            ...
            doesEncompass(
                addr, (1024 * 4096), kargs->kernelSegments[i].physLoc
            )) {
            return true;
        }
    }
    // do _we_ use it?
    if (doesEncompassRange(
            (uint64_t)&_tramp_start, (uint64_t)&_tramp_end, addr
        ) ||
        doesEncompassRange(
            (uint64_t)&_tramp_start, (uint64_t)&_tramp_end, addr + (1024 * 4096)
        ) ||
...
extern "C" void _start(KernelArgs* args, uint64_t kEntryVAddr) {
    ...
    // identity map wherever we are, and map 48MiB to idk 50MiB to KERNEL_OFFSET
    // + 48MiB and use the recursive mapping trick
    //
    ...
    // make a PDP table
    memory::PageDirPointer* pageMapLowPDPD =
        (memory::PageDirPointer*)nextFreeMem;
    ...
    // bind the table
    // it resolves to the most restrictive perms, so as long as we set them on
    // the lower levels we can use full perms higher up
    ...
    // this does mean if we try to access high kernel memory we will simply Run
    // Out of RAM so we gotta be careful user processes _should_ permission
    // error tho so we good on that map the highest 2GiB empty the rest of the
    // map first
    for (int i = 0; i < 510; i++) {
        kernelPDP[i].clear();
    }
    ...
    //
    // This implementation will NOT work if it lies on a GiB boundary. we should
    // probably do smth about that
    // TODO: see above
    //
    ...
    uint64_t fb_pde_idx_end = PDE_IDXOF(
        ((uint64_t)kargs->framebuffer +
         (kargs->fbPitch * kargs->fbHeight * sizeof(unsigned int)))
    );

    uint64_t addrHigh =
        ((1 << 30) * fb_pdp_idx) | (((uint64_t)1 << 39) * fb_pml4_idx);
    ...
    kargs->memDesc =
        (MemoryDescriptor*)((uint64_t)kargs->memDesc + KERNEL_OFFSET);
    kargs->kernelSegments =
        (KernelSegmentLoc*)((uint64_t)kargs->kernelSegments + KERNEL_OFFSET);
